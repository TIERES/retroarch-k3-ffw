   ...
   if (menu_materialui_icons_enable)
   {
      switch (type)
      {
         ...
         default:
            ...
            else if (string_is_equal(label, msg_hash_to_str(MENU_ENUM_LABEL_KAILLERA)))
            {
               node->icon_texture_index = MUI_TEXTURE_KAILLERA;
               node->icon_type = MUI_ICON_TYPE_INTERNAL;
            }
