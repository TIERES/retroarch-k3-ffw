...
static LRESULT win32_menu_loop(HWND owner, WPARAM wparam)
{
   ...
   switch (mode)
   {
      ...
      case ID_M_TAKE_SCREENSHOT:
         ...
         break;
      case ID_KAILLERA:
         command_event(CMD_EVENT_KAILLERA, NULL);
         break;
      case ID_M_QUIT:
      ...
      unsigned *width, unsigned *height)
{
   ...
   if (kailleraInitialised && g_win32_flags & WIN32_CMN_FLAG_QUIT) {
      runloop_msg_queue_push("Close Kaillera window first.", 0, 120, false, NULL, MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
      g_win32_flags &= ~WIN32_CMN_FLAG_QUIT;
   }
...
static enum msg_hash_enums menu_id_to_label_enum(unsigned int menuId)
{
   switch (menuId)
   {
      ...
      case ID_KAILLERA:
         return MENU_ENUM_LABEL_VALUE_INPUT_META_KAILLERA;
      case ID_M_MUTE_TOGGLE:
      ...
      bool fullscreen, bool windowed_full, void *rect_data)
{
   ...
   if (!fullscreen || windowed_full)
   {
      ...
      if (!fullscreen && ui_menubar_enable)
      {
         ...
         mInfo.dwStyle = kailleraInitialised ? MNS_MODELESS : 0; //do not freeze the game while checking menu on kaillera netplay
         SetMenuInfo(menuItem, &mInfo);

         SendMessage(main_window.hwnd, WM_NCCALCSIZE, FALSE, (LPARAM)&rc_temp);
         g_win32_resize_height = *height += rc_temp.top + rect->top;
         SetWindowPos(main_window.hwnd, NULL, 0, 0, *width, *height, SWP_NOMOVE | SWP_NOACTIVATE);
      }
#endif

      ShowWindow(main_window.hwnd, SW_RESTORE);
      UpdateWindow(main_window.hwnd);
      /* do not SetForegroundWindow when we drop game from kaillera */
