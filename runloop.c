...
int stop_execute_shit = 0;
unsigned int current_core_frame = 0;
      ...
      retro_time_t current_time)
{
   ...
   if (kailleraInitialised) is_focused = true;
#ifdef HAVE_MENU
   ...
   */
   ...
   /* Check quit hotkey */
   {
      ...
      if (kailleraInitialised) trig_quit_key = false;
...
bool load_content_and_core()
{
   ...
   task_push_load_content_with_new_core_from_companion_ui(cpath,
      fname, NULL, NULL, NULL, &content_info, NULL, NULL);
   return true;
}
...
void core_run(void)
{
...
#endif
         netjoy_ex[0][0] = kailleraCommands;
         kailleraCommands = 0;
      }

      if (kaillera_swap_cntr)
         kaillera_swap_cntr--;

      if (kailleraSwitch)
         command_event(CMD_EVENT_FULLSCREEN_TOGGLE, NULL);

      while ((kailleraWaitSaveLoad || kailleraSwitch) && kailleraNetplay) {	//for multithread cases?
#ifdef N02_LINUX
         usleep(1000);
#else
         Sleep(1);
...
#ifdef KAILLERA_DEFAULT
         switch (kailleraPacketSize)
         {
         case KAILLERA_DEFAULT:
         {
            if (kailleraSyncData(netjoy, 4) != -1)
            {
               for (int i = 0; i < kNumPlayers; i++) {
                  if (netjoy[i][0]) {
                     kReceivedCommand = netjoy[i][0];
                     netjoy[i][0] = 0;
                  }
                  joy[i][1] = netjoy[players[i].playerNumber][1];
               }
               //kaillera_buttons_write = 1; //sigh
            }
#if !defined(N02_WIN32) && !defined(N02_LINUX)
            else {
               EndKailleraGame(); // if use n02 callbacks don't need to end game from here
               // or end game will be called twice
            }
#endif
            break;
         }
         case KAILLERA_NEED_ANALOG:
         default:
         {
#endif
            if (kailleraSyncData(netjoy_ex, 12) != -1)
            {
               for (int i = 0; i < kNumPlayers; i++) {
                  if (netjoy_ex[i][0]) {
                     kReceivedCommand = netjoy_ex[i][0];
                     netjoy_ex[i][0] = 0;
                  }
                  for (int k = 1; k < 6; k++)
                  {
                     joy[i][k] = netjoy_ex[players[i].playerNumber][k];
                  }
               }
               //kaillera_buttons_write = 1; //sigh
            }
   ...
   if (kReceivedCommand) {
      ...
      case COMMAND_SWAP:
      {
         ...
         kaillera_swap_cntr = 60;
         int n1 = (kReceivedCommand >> 12) & 0xF;
         int n2 = (kReceivedCommand >> 8) & 0xF;
         if ((n1 > 0 && n1 <= kNumPlayers) && (n2 > 0 && n2 <= kNumPlayers))
         {
            int p1 = players[n1 - 1].playerNumber;
            int p2 = players[n2 - 1].playerNumber;
            players[n1 - 1].playerNumber = p2;
            players[n2 - 1].playerNumber = p1;
         }
         kReceivedCommand = 0;
         break;
      }
      case COMMAND_TAKE:
      {
         ...
         int n2 = (kReceivedCommand >> 8) & 0xF;
         if ((n1 > 0 && n1 <= kNumPlayers) && (n2 > 0 && n2 <= kNumPlayers)) {
            players[n2 - 1].playerNumber = n1 - 1;
         }
         kReceivedCommand = 0;
         break;
      }
      case COMMAND_SWAP_RESET:
      {
         for (int i = 0; i < kNumPlayers; i++)
