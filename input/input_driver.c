...
unsigned track_0_port_cntr;
unsigned _trackal_device[100];
unsigned _trackal_idx[100];
unsigned _trackal_id[100];
...
int16_t joy[MAX_INPUTS][6] = { {0},{0},{0},{0},{0},{0},{0},{0} };
#ifdef KAILLERA_DEFAULT
int16_t netjoy[MAX_INPUTS][2] = { {0},{0} };
#endif
//int16_t netjoy_min[MAX_INPUTS][1];
int16_t netjoy_ex[MAX_INPUTS][6] = { {0},{0},{0},{0},{0},{0},{0},{0} };
...
void input_state_kaillera(unsigned port, unsigned device,
   unsigned idx, unsigned id)
{
   rarch_joypad_info_t joypad_info;
   ...
   if ((device == RETRO_DEVICE_ANALOG) &&
      (ret == 0))
   {
      if (input_st->libretro_input_binds[port])
      {
         if (idx == RETRO_DEVICE_INDEX_ANALOG_BUTTON)
         {
            if (id < RARCH_FIRST_CUSTOM_BIND)
            {
               bool valid_bind = (*input_st->libretro_input_binds[port])[id].valid &&
               ...
               if (valid_bind)
               {
                  if (sec_joypad)
                     ret = input_joypad_analog_button(
                        ...
                        sec_joypad, &joypad_info,
                        id,
                        &(*input_st->libretro_input_binds[port])[id]);
   ...
   if (!input_blocked)
   {
      ...
      if (bitmask_enabled)
      {
         ...
         for (i = 0; i < RARCH_FIRST_CUSTOM_BIND; i++)
            if (input_state_device(input_st,
               settings, handle,
               input_analog_dpad_mode, ret, port,
               device, idx, i, true))
               port_result |= (1 << i);
      }
      else
   ...
   case RETRO_DEVICE_JOYPAD:
   {
      if (bitmask_enabled)
      {
         if (id != 0xD) //fceumm hack with id= 0xD (zapper?)
         {
#ifdef KAILLERA_DEFAULT
            netjoy[0][1] = port_result;
#endif
            netjoy_ex[0][1] = port_result;
            //netjoy_min[0][0] = port_result;
         }
      }
      else
      {
...
#ifdef KAILLERA_DEFAULT
         netjoy[0][1] &= ~(1 << id);
         netjoy[0][1] |= port_result << id;
#endif
         netjoy_ex[0][1] &= ~(1 << id);
         netjoy_ex[0][1] |= port_result << id;
         //netjoy_min[0][1] &= ~(1 << id);
         //netjoy_min[0][0] |= port_result << id;
      }
      break;
   }
   case RETRO_DEVICE_ANALOG:
   {
      //netjoy_ex[0][0x2 + (idx & 1)] &= ~(0xFF << ((id & 1) * 8));	//clear bit
      //netjoy_ex[0][0x2 + (idx & 1)] |= ((port_result & 0xFF00) >> 8) << ((id & 1) * 8);  //n64 thing        8000 left
      if (idx == 0)
      {
         netjoy_ex[0][0x2 + id] = port_result;
      }
      else
      {
         netjoy_ex[0][0x4 + (id & 1)] = port_result;
      }
      break;
   }
   default:
      ...
      input_keyboard_line_t *state, uint32_t character)
{
   ...
   else if (c == '\b' || c == '\x7f') /* 0x7f is ASCII for del */
   {
      if (state->ptr)
      {
         ...
         //   printf("delete %02x\n", state->buffer[state->ptr-1]);
         if (state->ptr)
            if ((state->buffer[state->ptr - 1] & 0xFF) > 0xCF)
            {
               {
                  memmove(state->buffer + state->ptr - 1,
                     state->buffer + state->ptr,
   ...
   else if (ISPRINT(c) || ((character > 167) && (character < 256)))
   {
      /* Handle left/right here when suitable */
      char *newbuf = (char*)
         realloc(state->buffer, state->size + 3);
      if (!newbuf)
         return false;
      ...
      memmove(newbuf + state->ptr + 1,
            newbuf + state->ptr,
            state->size - state->ptr + 1);
      if ((character > 167) && (character < 256))
      {
         char in[256];
         in[0] = character;
         char utf8[256];
         ...
         //printf("%02x, %x\n", out[0], character);
         newbuf[state->ptr] = out[0];
         newbuf[state->ptr + 1] = out[1];
         state->ptr++;
         state->size++;
         state->ptr++;
         state->size++;
   ...
   if (kailleraNetplay) {
      ...
      while ((mapped_port = *(input_remap_port_map++)) < MAX_USERS)
      {
         ...
         case RETRO_DEVICE_JOYPAD:
         {
            if (bitmask_enabled)
            {
               result = joy[mapped_port][1];
            }
            else
            {
               id &= 0xF;
               result = (joy[mapped_port][1] >> id) & 1;
            }
            break;
         }
         case RETRO_DEVICE_ANALOG:
         {
            //result = (joy[mapped_port][0x2 + (idx & 1)] << (8 - ((id & 1) * 8))) & 0xFF00;
            if (idx == 0)
            {
               result = joy[mapped_port][0x2 + id];
            }
            else
            {
               result = joy[mapped_port][0x4 + (id & 1)];
            }
            break;
         }
         default:
      ...
      if (current_core_frame <= INIT_FRAMES)
         result = 0;

      //kaillera_buttons_write = 0; //dirty hack for N64 30fps

      /* setup our devices ids to check them later for kaillera input send */
      // somes cores in different games start checking input in different frames and different number of times...
      // how much frames do we check?
      if (port == 0 && current_core_frame > 1 && current_core_frame < INIT_FRAMES)
      {
         if (device == RETRO_DEVICE_JOYPAD || device == RETRO_DEVICE_ANALOG) {
            bool write = true;
            for (int i = 0; i < track_0_port_cntr; i++) {
               if (_trackal_device[i] == device &&
                  _trackal_idx[i] == idx &&
                  _trackal_id[i] == id)
                  write = false; // skip if we already have those values
            }
            if (write) {
               _trackal_device[track_0_port_cntr] = device;
               _trackal_idx[track_0_port_cntr] = idx;
               _trackal_id[track_0_port_cntr] = id;
               track_0_port_cntr++;
               //RARCH_DBG("Nums of writes: %d CF[%i] dev[%i] idx[%i] id[%i]\n",\
                     track_0_port_cntr, current_core_frame, device, idx, id);
            }
         }
      }
   }
