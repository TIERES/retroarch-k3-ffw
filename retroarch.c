...
int kailleraCommands = 0;
volatile int kailleraInitialisedInternal = 0;
int kailleraWaitSaveLoad = 0;
int kaillera_swap_cntr = 0;
...
bool command_event(enum event_command cmd, void *data)
{
   ...
   switch (cmd)
   {
      ...
      case CMD_EVENT_LOAD_STATE:
         {
            if (kailleraNetplay) {
               if ((kReceivedCommand != COMMAND_LOAD_STATE)) {
                  kailleraCommands = COMMAND_LOAD_STATE;
                  break;
               }
               settings->ints.state_slot = 0;
               kailleraWaitSaveLoad = 1;
               kReceivedCommand = 0;
            }
#ifdef HAVE_CHEEVOS
      ...
      case CMD_EVENT_RESET:
         if ((kReceivedCommand != COMMAND_RESET) && kailleraNetplay)
         {
            kailleraCommands = COMMAND_RESET;
            break;
         }
         kReceivedCommand = 0;
      ...
      case CMD_EVENT_SAVE_STATE_TO_RAM:
         {
            if (kailleraNetplay)
            {
               if (kReceivedCommand != COMMAND_SAVE_STATE) {
                  kailleraCommands = COMMAND_SAVE_STATE;
                  break;
               }
               else {
                  settings->ints.state_slot = 0;
      ...
      case CMD_EVENT_FULLSCREEN_TOGGLE:
         {
            if (kailleraNetplay && !kailleraSwitch)
            {
               kailleraSwitch++;
               break;
            }
            else if (kailleraNetplay && (kailleraSwitch != 1))
            {
               break; //ready
            }
            audio_driver_state_t
...
static void global_free(struct rarch_state *p_rarch)
{
   ...
   if (kailleraNetplay) runloop_st->flags |= RUNLOOP_FLAG_IS_SRAM_LOAD_DISABLED;
#ifdef HAVE_PATCH
   p_rarch->flags    &= ~(
